...
class PromotedProductCache {
  ...
  getOrGenerateOrder(promotedProducts) {
    // Check if cache is still valid
    if (this.isValid()) {
      console.log('[PromotedCache] Using cached order (age: ' + 
        Math.floor((Date.now() - this.cacheTimestamp) / 1000) + 's)');
      return this.cache;
    }

    // Generate new randomized order
    console.log('[PromotedCache] Generating new randomized order');
    ...
    console.log('[PromotedCache] Randomized products by tier:', tierCounts);
    console.log('[PromotedCache] Cache will expire in 2 minutes');
  ...
  clear() {
    this.cache = null;
    this.cacheTimestamp = null;
    console.log('[PromotedCache] Cache cleared');
  }
